...
void prepareObject(ShaderProgram *shaderProgram, GraphicalObject &object) {
	...
	object.bufNormals=makeBuffer(&object.normals[0], object.vertices.size(), sizeof(float)*4);//VBO z wektorami normalnymi wierzchołków
...
void initOpenGLProgram(GLFWwindow* window) {
	...
	printf("%d %d %d\n",konar.vertices.size(),konar.uvs.size(),konar.normals.size());
...
void drawObject(GLuint vao, ShaderProgram *shaderProgram, mat4 mP, mat4 mV, mat4 mM, float angle_x, float angle_y) {
	...
	//glUniform4fv(shaderProgram->getUniformLocation("light0"),1,glm::value_ptr(vec4(0,cos(angle_x)*1000,cos(angle_y)*-1000,1)));
	glUniform4fv(shaderProgram->getUniformLocation("light1"),1,glm::value_ptr(vec4(0,1000,1000,1)));
