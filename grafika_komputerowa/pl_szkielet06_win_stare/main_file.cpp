...
void prepareObject(ShaderProgram *shaderProgram) {
	...
	bufNormals=makeBuffer(&uvsGrass[0], verticesGrass.size(), sizeof(float)*4);//VBO z wektorami normalnymi wierzchołków
	bufTexCoords=makeBuffer(&uvsGrass[0], verticesGrass.size(), sizeof(float)*2);//VBO ze współrzędnymi teksturowania
...
void initOpenGLProgram(GLFWwindow* window) {
	...
	tex0=readTexture("bark.png");
	tex1=readTexture("metal_spec.png");

	bool res = loadOBJ("konar.obj", verticesGrass, uvsGrass, normalsGrass);
...
void drawScene(GLFWwindow* window, float angle_x, float angle_y) {
    ...
    for(int i=0;i<10;i++){
        ...
        //M = scale(M,vec3(konar_size*10.0f/(rand()%30),konar_size/3,konar_size/3));
