...
void key_callback(GLFWwindow* window, int key,
	...
	if(action == GLFW_REPEAT){
        if(key == GLFW_KEY_W) camera_z += 0.1;
        if(key == GLFW_KEY_S) camera_z -= 0.1;
        if(key == GLFW_KEY_A) camera_x += 0.1;
        if(key == GLFW_KEY_D) camera_x -= 0.1;
	}
}
...
int main(void)
{
	...
	window = glfwCreateWindow(500, 500, "OpenGL", nullptr, nullptr);  //Utwórz okno 500x500 o tytule "OpenGL" i kontekst OpenGL.
